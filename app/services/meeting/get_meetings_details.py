...
def get_meeting_details_service(user_id: int, db: Session) -> list[MeetingOverviewResponse]:
    try:
        ...
        audio_files_db = get_meeting_audio_fiels_by_meeting_ids_repo(meeting_ids= cast(list, meetings_id), db=db)

        audio_files_by_meeting_id: dict[int, list] = defaultdict(list)
        for audio_file in audio_files_db:
            audio_files_by_meeting_id[cast(int, audio_file.meeting_id)].append(audio_file)
        ...
        for meeting in meetings:
            ...
            pending_count = 0
            meeting_audio_files = audio_files_by_meeting_id.get(cast(int, meeting.id), [])
